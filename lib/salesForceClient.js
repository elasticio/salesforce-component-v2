...
class SalesForceClient {
  ...
  async describe(sobject) {
    if (describedObject
      && describedObject.sobject === (sobject || this.configuration.sobject)
      && process.env.NODE_ENV !== 'test') return describedObject.description;
    const description = await this.connection.describe(sobject || this.configuration.sobject);
    describedObject = {
      sobject: sobject || this.configuration.sobject,
      description,
    };
    return description;
  }
  ...
  async queryEmitAll(query) {
    ...
    await new Promise((resolve, reject) => {
      const response = this.connection.query(query)
        ...
        .execute({ scanAll: this.configuration.includeDeleted });
    });
    return result;
  }
  ...
  async queryEmitBatch(query) {
    ...
    await new Promise((resolve, reject) => {
      const response = this.connection.query(query)
        ...
        .execute({ autoFetch: true, maxFetch, scanAll: this.configuration.includeDeleted });
    });
    return results;
  }
  ...
  async sobjectDownload(options) {
    const sobject = options.sobject || this.configuration.sobject;
    const { id, fieldName } = options;
    return this.connection.sobject(sobject).record(id).blob(fieldName);
  }
    ...
    try {
      await new Promise((resolve, reject) => {
        this.connection.sobject(sobject)
          ...
          .execute({ autoFetch: true, maxFetch: limit, scanAll: includeDeleted });
      });
    } catch (e) {
  ...
  async bulkQuery(query) {
    const recordStream = await this.connection.bulk.query(query);
    return recordStream
      .on('error', (err) => {
        throw err;
      })
