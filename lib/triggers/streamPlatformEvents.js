 ...
 */
async function processTrigger(msg, configuration, snapshot) {
  context = this;
  const currentDate = new Date().toJSON().slice(0, 10);
  if (status === 'online') {
    if (configuration.recreateSubscription && !(snapshot?.lastSubscription === currentDate)) {
      this.logger.info('Going to recreate subscription');
      fayeClient = undefined;
      await this.emit('snapshot', { lastSubscription: currentDate });
    } else {
  ...
  if (!fayeClient || status === 'down') {
    ...
    status = 'online';
    if (configuration.recreateSubscription && !snapshot?.lastSubscription) await this.emit('snapshot', { lastSubscription: currentDate });
