...
const isAuthError = (error) => {
  ...
  if (error.message?.includes('UNAUTHENTICATED')) return true;
  if (error.metadata) {
    const errorCode = error.metadata.get('error-code') || error.metadata.get('error_code');
    if (Array.isArray(errorCode) && errorCode.includes('sfdc.platform.eventbus.grpc.service.auth.error')) return true;
    if (typeof errorCode === 'string' && errorCode.includes('sfdc.platform.eventbus.grpc.service.auth.error')) return true;
  }
  return false;
};
...
async function processTrigger(msg, cfg, snapshot) {
  ...
  try {
    ...
    try {
      ...
      if (isAuthError(err) && authFailureRetry <= MAX_AUTH_RETRIES) {
    ...
    try {
      ...
      if (isAuthError(err) && authFailureRetry <= MAX_AUTH_RETRIES) {
        context.logger.warn(`AuthFailure error occurred during keepalive wait, lets call processTrigger one more time (retry ${authFailureRetry} of ${MAX_AUTH_RETRIES})`);
        authFailureRetry++;
        refreshTokenNeeded = true;
        await reconnectHandler(2000, cfg);
      } else if (!err.message?.includes('INVALID_ARGUMENT') && keepaliveFailureRetry <= MAX_CONNECT_RETRIES) {
    ...
    eventEmitter.on('error', (error) => {
      if (isAuthError(error)) {
        context.logger.warn('AuthFailure error occurred in stream, refreshing token and reconnecting');
        refreshTokenNeeded = true;
        authFailureRetry = 1;
      }
      reconnectHandler(2000, cfg);
