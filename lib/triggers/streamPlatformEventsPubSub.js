...
const healthCheck = (cfg) => {
  context.logger.info(`last - ${client.getLastKeepAlive()}, current - ${(new Date().getTime() - 300000)}, diff ${client.getLastKeepAlive() - (new Date().getTime() - 300000)}, healthCheckRunning - ${healthCheckRunning}`);
  if (client) {
...
async function processTrigger(msg, cfg, snapshot) {
  ...
  try {
    ...
    if (err.message.includes('UNAUTHENTICATED') && authFailureRetry <= MAX_AUTH_RETRIES) {
      ...
      context.logger.warn(`Failed to connect to Salesforce: ${err.message}`);
