...
exports.process = async function processTrigger(_msg, cfg, snapshot = {}) {
  ...
  if (pageSize === null || pageSize === undefined) pageSize = MAX_FETCH;
  ...
  if (pageSize > MAX_FETCH || isNumberNaN(pageSize) || Number(pageSize) <= 0) throw new Error(`"Size of Polling Page" must be valid number between 1 and ${MAX_FETCH}`);
