...
class PubSubClient {
  ...
  async connect(accessToken, instanceUrl) {
    const rootCert = await fs.readFile('/etc/ssl/certs/ca-certificates.crt');
  ...
  async subscribe(numRequested, fromReplayId) {
    ...
    this.subscription.on('data', (data) => {
      try {
        ...
        this.logger.warn(`Failed to decode incoming data, skipping: ${err.message}`, {
          error: err?.name,
          stack: err?.stack,
          hasEvents: !!data.events,
          eventsCount: data.events?.length,
          hasLatestReplayId: !!data.latestReplayId,
        });
      }
    });
