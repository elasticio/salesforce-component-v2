...
exports.processMeta = async function processMeta(meta, metaType, lookupField) {
  ...
  if (metaType === META_TYPES_MAP.create) {
    fields = await fields.filter((field) => field.createable);
  } else if (metaType === META_TYPES_MAP.upsert) {
...
module.exports.formatWhereCondition = (type, lookupField, lookupValue) => ((['date', 'datetime'].includes(type)
  || TYPES_MAP[type] === 'number'
  || TYPES_MAP[type] === 'boolean')
  ? `${lookupField} = ${lookupValue}`
  : `${lookupField} = '${lookupValue}'`);

module.exports.logAndThrowError = (errMsg, logger) => {
